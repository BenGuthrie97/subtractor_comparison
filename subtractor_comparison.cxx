...
int main(){
  ...
  for (int j = 0; j < root_events.size(); j++){
   ...
   // cout << endl;
    ...
    vector< vector<double> > jets_in_event = all_signal_jets[j];
   ...
   //   cout << "pt = " << params[0] << ", rap = " << params[1] << ", phi = " << params[2] << ", E = " << params[3] << endl;
   // }
   ...
   // cout << endl;
    ...
    vector< vector<int> > CS_root_key;
    CS_root_key = match_jets(signal_coors,CS_coors);
    vector < vector<int> > ICS_root_key;
    ICS_root_key = match_jets(signal_coors, ICS_coors);

    //cout << "Here are the jet matching keys." << endl;
    //cout << "CS_root_key: " << endl;
    //for (int i = 0; i < CS_root_key.size(); i++){
    //  cout << CS_root_key[i][0] << " " << CS_root_key[i][1] << endl;
    //}
    //cout << "ICS_root_key: " << endl;
    //for (int i = 0; i < ICS_root_key.size(); i++){
    //  cout << ICS_root_key[i][0] << " " << ICS_root_key[i][1] << endl;
    //}
    ...
    results << corrected_CS_root_jet_params.size() - jets_in_event.size() << " " << corrected_ICS_root_jet_params.size() - jets_in_event.size() << endl;
    for (int i = 0; i < jets_in_event.size(); i++){
      vector<double> jet = jets_in_event[i];
      int CS_tag = CS_root_key[i][1];
      vector<double> CSjet = corrected_CS_root_jet_params[CS_tag];
      results << abs(jet[0] - CSjet[0])/jet[0] << " " << abs(jet[1] - CSjet[1])/abs(jet[1]) << " " << phi_dist_alt(jet[2],CSjet[4])/abs(jet[2]) << " " << abs(jet[3]-CSjet[2])/jet[3] << endl;
    }
    for (int i = 0; i < jets_in_event.size(); i++){
      vector<double> jet = jets_in_event[i];
      int ICS_tag = ICS_root_key[i][1];
      vector<double> ICSjet = corrected_ICS_root_jet_params[ICS_tag]; 
      results << abs(jet[0] - ICSjet[0])/jet[0] << " " << abs(jet[1] - ICSjet[1])/abs(jet[1]) << " " << phi_dist_alt(jet[2],ICSjet[4])/abs(jet[2]) << " " << abs(jet[3]-ICSjet[2])/jet[3] << endl;
    }
